...
def loop_through_ids(fasta_dict, uids, H_group,  input_dir, output_dir, hhblits, hhalign, uniprot, address):
	'''Try to make alignments with each uid used once that fulfill criteria.
	'''
	...
	for i in range(len(uids)):
		#Check if .hhm exists
		if not os.path.isfile(output_dir+uids[i]+'.hhm'):
			#Make HMM
			run_hhblits(uids[i], input_dir, hhblits, uniprot)
		#Check if .pdb exists
		if not os.path.isfile(output_dir+uids[i]+'.pdb'):
			#Write to selected uid file
			uid_file.write(uids[i]+'\n')
	#Align
	(latest_pos, identities) = align(uids, output_dir, H_group, hhalign, identities)
...
def align(selected_uids, output_dir, group, hhalign, identities):
	...
	for i in range(0, end, 2):
		#Make sure there is something after i
		j = i+1
		if end < j+1:
			break
		
		structure_i =output_dir+selected_uids[i]+'.hhm' #Get structure i
		#structure j is the uid after i
	...
	write_to_file(output_dir, parsed_output)

	return(i, identities)


def write_to_file(output_dir, parsed_output):
...
group = input_dir.split('/')[-1] #Get topology group (last part of path)
...
loop_through_ids(fasta_dict, uids, group, input_dir,  output_dir, hhblits, hhalign, uniprot, address)
